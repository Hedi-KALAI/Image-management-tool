...
public class Crypt extends Filtres {
    public Crypt(){}
    public Crypt(ImageView imageView,String pass){
        ...
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                double r = pixelReader.getColor(y, x).getRed();
                double g = pixelReader.getColor(y, x).getGreen();
                double b = pixelReader.getColor(y, x).getBlue();
                x++;
                y++;
